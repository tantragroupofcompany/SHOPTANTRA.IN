-- SHOPTANTRA marketplace migration
-- Date: 2026-08-25
-- Additive/backwards-compatible only. No columns or tables are dropped.
-- Apply with:  npx prisma migrate deploy   (PostgreSQL)   OR   psql $DATABASE_URL -f migration.sql

-- 1. Seller: store the Razorpay linked-account / settlement account identifier
ALTER TABLE "Seller" ADD COLUMN IF NOT EXISTS "razorpayLinkedAccountId" TEXT;

-- 2. Product: optional product-specific commission override (%)
ALTER TABLE "Product" ADD COLUMN IF NOT EXISTS "commissionPercent" DOUBLE PRECISION;

-- 3. Order: refund bookkeeping
...
-- 4. Payment: marketplace split amounts (platform vs seller transfer)
...
-- 5. Commission: enrich ledger with transfer/failure detail
...
-- 6. CommissionRule: configurable GLOBAL / CATEGORY / PRODUCT commission rules
...
-- 7. SellerSettlement: automatic transfer ledger
