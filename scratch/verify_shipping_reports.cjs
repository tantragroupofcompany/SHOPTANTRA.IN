// scratch/verify_shipping_reports.cjs
...
async function main() {
  ...
  try {
    ...
    // Inline simple barcode and QR generators
    const generateBarcodeSVG = (text) => `<svg>mock-barcode-for-${text}</svg>`;
    const getQRCodeUrl = (data) => `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(data)}`;
