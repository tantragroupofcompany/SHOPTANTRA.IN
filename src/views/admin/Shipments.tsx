...
const Shipments = () => {
  ...
  const filteredShipments = shipments.filter(ship => {
    ...
    if (searchQuery) {
      ...
      const storeMatch = ship.seller?.storeName.toLowerCase().includes(q);
      return numMatch || orderMatch || awbMatch || customerMatch || storeMatch;
    }
    return true;
  });
  ...
  const columns = [
    ...
    },
    {
      key: 'sellerStore',
      header: 'Seller',
      render: (row: Shipment) => <span className="text-gray-700 font-medium">{row.seller?.storeName || 'N/A'}</span>
    },
    {
      key: 'customerName',
      header: 'Customer',
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by Shipment ID, Order, Seller, Customer..."
        ...
        {selectedShipment && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-150 text-xs">
              ...
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase">Seller Merchant</p>
                <p className="text-sm font-semibold text-gray-900">{selectedShipment.seller?.storeName}</p>
              </div>
              <div>
            ...
            <div className="flex gap-2">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="bg-white hover:bg-gray-100 border border-gray-200 text-gray-700 text-xs px-4 py-2 rounded font-bold inline-flex items-center gap-1.5 animate-pulse"
              >
